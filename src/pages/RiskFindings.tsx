...
import { mockData, Risk } from '../mockData';
...
const RiskFindings = () => {
  ...
  const filteredRisks = mockData.risks.filter((risk) => {
  ...
  const categories = ['ALL', ...Array.from(new Set(mockData.risks.map((r) => r.category)))];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">
          Review and remediate security misconfigurations and risks
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
        ...
        <div className="mt-4 flex items-center gap-2 text-sm text-slate-400">
          <span>Showing {filteredRisks.length} of {mockData.risks.length} findings</span>
        </div>
      </div>
                  ...
                  <>
                    ...
                    >
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border ${severityConfig.bg} ${severityConfig.border}`}
                        >
